...
SoftShadowShader::SoftShadowShader(MyRender* render)
{
	m_render = render;
	m_shader = nullptr;
...
bool SoftShadowShader::Init()
{
	...
	if (!m_shader->CreateShader(L"softshadow.vs", L"softshadow.ps"))
		return false;
...
void SoftShadowShader::Render(int indexCount, CXMMATRIX worldMatrix, CXMMATRIX WVP, CXMMATRIX WVPlight, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* depthMapTexture, Light& light, CXMMATRIX WVPlight2, ID3D11ShaderResourceView* depthMapTexture2, Light& light2)
{
	MatrixBufferType cb;
